...
def load_mapping(filepath: str) -> dict:
    ...
    with open(filepath, 'r') as file:
        return {
            ...
            for line in file if ':' in line.strip()
...
def process_mapping(mapping: dict, old_col: str, new_col: str) -> dict:
    ...
    """
    return {
        clean_string(k): clean_string(v) for k, v in mapping.items()
    }
...
def clean_text(text: str) -> str:
    ...
    text = re.sub("\s+", " ", text).strip()  # Remove repeated, leading, and trailing spaces
    return text
...
def get_preprocessed_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    entity_mapping = process_mapping(load_mapping(ENTITY_FIXED_FILEPATH), 'OLD_ENTITY', 'NEW_ENTITY')
    vendor_mapping = process_mapping(load_mapping(VENDOR_FIXED_FILEPATH), 'OLD_VENDOR', 'NEW_VENDOR')

    # Rename and update columns
    processed_df = df.rename(columns={
        'VENDOR': 'TYPO_MIXED_VENDOR',
        'ENTITY': 'TYPO_MIXED_ENTITY'
    }).copy()
    ...
    processed_df['TENDER_DESCRIPTION'] = processed_df['TENDER_DESCRIPTION'].fillna('').str.lower()
    ...
    processed_df['TENDER_DESCRIPTION'] = processed_df['UNCLEANED_TENDER_DESCRIPTION'].fillna('').apply(clean_text)
