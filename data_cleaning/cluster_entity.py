...
def assign_cluster_names(data, texts, clusters, cluster_names):
    ...
    entity_to_cluster_name = dict(
        zip(cluster_mapping["Entity"], cluster_mapping["ClusterName"])
    )
...
def display_cluster_groups(cluster_mapping):
    ...
    for cluster_num in sorted(cluster_mapping["Cluster"].unique()):
        ...
        cluster_entities = cluster_mapping[cluster_mapping["Cluster"] == cluster_num][
            "Entity"
        ].tolist()
...
def analyze_entity_clusters(
    data, num_kmeans_clusters=15, num_agglomerative_clusters=15
):
    """
    ...
    data, cluster_mapping = assign_cluster_names(
        data, texts, agg_clusters, cluster_names
    )
